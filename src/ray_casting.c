...
void	floor_casting(t_game *game)
{
	...
	while (y < H)
	{
		...
		while (x < W)
		{
			int tx = (int)(64 * (floor->floorX - (int)floor->floorX)) & (64 - 1);
			int ty = (int)(64 * (floor->floorY - (int)floor->floorY)) & (64 - 1);

			floor->floorX += floor->floorStepX;
			floor->floorY += floor->floorStepY;

			game->ray->color = game->ray->texture[3][64 * ty + tx];
			game->ray->color = (game->ray->color >> 1) & 8355711;
			game->ray->buf[y][x] = game->ray->color;
			// printf("????\n");

			game->ray->color = game->ray->texture[6][64 * ty + tx];
			game->ray->color = (game->ray->color >> 1) & 8355711;
			game->ray->buf[H - y - 1][x] = game->ray->color;
			x++;
		}
		y++;
	}
